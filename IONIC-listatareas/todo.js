...
function addEditItem(index = false) {
    ...
    modal.component.innerHTML = `
        <ion-header>
            <ion-toolbar>
                <ion-title>Cosas ha realizar - `+(index !== false ? 'Editar tarea' : 'Nueva tarea')+`</ion-title>
        ...
        <ion-content>
            <ion-list>
                ...
                <ion-item>
                    <ion-label position="floating">Escriba la tarea a realizar</ion-label>
                    <ion-input value="`+item.text+`"></ion-input>
                </ion-item>
                <ion-item>
                <ion-label>Avance tarea</ion-label>
                <ion-range value=`+item.text2+`"min="0" max="10" snaps>
    ...
    modal.component.querySelector('[color="primary"]').addEventListener('click', () => {
        ...
        let newText2 = modal.component.querySelector('ion-range').value;
        if (!newText.length) {
            error('No puede estar vacía la tarea');
        }
        else {

            let newItem = { text:newText, date:newDate, icon:newIcon ,text2:newText2};
